...
def get_available_formats_text(date_order: str) -> str:
    if date_order == "DMY":
        text = _("Бот поддерживает большое количество форматов дат, включая все "
                 "Международные форматы даты. Ваш поддерживаемый формат зависит от "
                 "настроек указанный внизу.\n\n"
                 "Ниже приведены примеры записи даты «<b>28 июня 2018 года</b>»\n\n"
                 "<b>Наиболее распространённые форматы:</b>\n"
                 "- д.м.гггг → 28.6.2018;\n"
                 "- д-м-гггг → 28-6-2018;\n"
                 "- д/м/гггг →  28/6/2018;\n"
                 ...
                 "- дд-мм-гггг → 28-06-2018;\n"
                 "- дд/мм/гггг → 28/06/2018;\n\n"
                 "<b>Также доступные форматы даты:</b>\n"
                 "- ддммгггг → 28062018;\n"
                 "- дд мм гггг → 28 06 2018;\n"
                 "- дд м гг → 28 6 18;\n"
                 "- дд М гг → 28 июня 18 года;")
    elif date_order == "MDY":
        text = _("Бот поддерживает большое количество форматов дат, включая все "
                 "Международные форматы даты. Ваш поддерживаемый формат зависит от "
                 "настроек указанный внизу.\n\n"
...
def get_date_order_text(date_order) -> str:
    avl_date_orders = [_("ДМГ"), _("ГМД"), _("МДГ")]
    if date_order == "DMY":
        date_order = avl_date_orders[0]
    elif date_order == "YMD":
        date_order = avl_date_orders[1]
    elif date_order == "MDY":
        date_order = avl_date_orders[2]
    return date_order
